...
def codon_mut_col(cscs: pd.DataFrame, func):
    cscs["pos"] += 1
    """remove unusable elements"""
    # 1. get rid of row that matches the condition "pos == 1" (starting position)
...
if __name__ == "__main__":
    ...
    if args.indep:
        ...
        cscs["indep"] = codon_mut_col(cscs, pos_indep_prob)
    if args.dep:
        with open(args.dep, "r") as json_fs:
            dep = json.load(json_fs)
        cscs["dep"] = codon_mut_col(cscs, pos_dep_weight)
